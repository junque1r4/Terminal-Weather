...
from API_KEY import (
    API_KEY as KEY,
)  # Put your API key in the in a python file with a constant named API_KEY
...
def read_user_args() -> argparse.Namespace:
    ...
    parser.add_argument("city", nargs="+", type=str, help="Enter the name of the city.")

    return parser.parse_args()


def weather_query(city: list) -> string:
    city_name = " ".join(city)
    url_encoded_city_name = parse.quote_plus(city_name)

    url = f"{BASE_API_URL}?q={url_encoded_city_name}&units=metric&APPID={KEY}"
...
def show_weather_data(data_in_json: dict) -> None:
